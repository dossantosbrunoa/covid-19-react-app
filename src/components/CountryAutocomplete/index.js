...
const CountryAutocomplete = ({ defaultSlug }) => {
    const { countryOptionsList, countryLoading } = useSelector((state) => state.covid);
    const [selectedCountry, setSelectedCountry] = useState(null);
    const dispatch = useDispatch();

    useEffect(() => {
        if(!!countryOptionsList && countryOptionsList.length > 0) {
            const defaultValue = countryOptionsList.find(c => c.slug === defaultSlug);
    ...
    return (
        ...
        <Autocomplete
            ...
            renderInput={(params) => <TextField {...params} variant="outlined" placeholder="Digite o nome do País" />}
        />
    )
}
